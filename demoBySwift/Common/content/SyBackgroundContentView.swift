//
//  SyBackgroundContentView.swift
//  demoBySwift
//
//  Created by sxm on 2017/3/21.
//  Copyright © 2017年 sxm. All rights reserved.
//

import UIKit

class SyBackgroundContentView: SyBasicContentView {
    ...
    public convenience init(specialWithAutoImplies implies: Bool) {
        ...
        highlightBackdropColor = UIColor.init(red: 22 / 255.0, green: 24 / 255.0, blue: 25 / 255.0, alpha: 1.0)
        if implies {
            let timer = Timer.scheduledTimer(timeInterval: 3.0, target: self, selector: #selector(SyBackgroundContentView.playImpliesAnimation(_:)), userInfo: nil, repeats: true)
